...
open class MustacheTemplateService(
    ...
    override fun years(yearMonths: List<YearMonth>): String {
        Assert.isTrue(
            ...
            "there must be at least one element in the ${YearMonth::class.java.name} collection."
        )
...
//                , "built" to Instant.now().toString()
    ...
    private fun buildMapForLink(link: Link): Map<String, Any> = mutableMapOf<String, Any>()
        .apply {
            BeanUtils.getPropertyDescriptors(Link::class.java)
                .forEach { pd -> this[pd.name] = pd.readMethod.invoke(link) }
            val hasTemplateMarkers =
                (listOf(URL_MARKER, ID_MARKER, DESC_MARKER).firstOrNull { link.description.contains(it) } != null)
    ...
    private fun buildHtmlForLink(template: String, href: String, pk: String, desc: String): String {
        ...
        mapOf(URL_MARKER to href, ID_MARKER to pk, DESC_MARKER to desc).forEach { (k, v) ->
            atomicReference.set(
                atomicReference.get().replaceAll(k, v)
            )
        }
