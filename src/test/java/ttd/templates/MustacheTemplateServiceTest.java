...
@ImportAutoConfiguration({MustacheAutoConfiguration.class,
	TemplateServiceAutoConfiguration.class})
...
public class MustacheTemplateServiceTest {
		...
		public void replace() {
				...
				String replaced = MustacheTemplateService.class.cast(ts).replace(content, find, replace);
				Assert.assertEquals(replaced, " <a class=\"link\" href=\"_URL_\" name =\"_ID_\" id=\"_ID_\">Treasury probing $25K Mnuchin flight from New York to DC | TheHill</a>");
		}
		...
		public void index() {
				...
				String index = this.ts.index(results);
